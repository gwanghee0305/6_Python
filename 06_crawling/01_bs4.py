...
#bs4 는 문자열을 DOM 트리처럼 다룰 수 있게 만들어줌
soup = BeautifulSoup(html, 'lxml')

print(f"title --> {soup.title.text if soup.title else '없음'}")

# 기존에 자바스크립트를 통해 DOM을 조작한 것 처럼
#      bs가 같은 역할을 함
...
# f-string 에서 !r 을 사용하면 repr()이 호출이 되어
#   숨겨진 공백(\n,\t 등)까지 그대로 출력해줌
