...
"""
print(" === 현재 상태에서 통계 조회 === ")
print(df[['close', 'volume', 'high']].describe())
...
# 동일한 데이터가 오염되어 있어도 N/A는 결측으로 확인이 되는데,
#   오염된 데이터가 확실하게 확인하기 어려움
# 데이터를 읽어올 때 결측으로 판단할 값들을 지정할 수 있음
# => na_values=['N/A','-',''] 옵션을 통해 결측을 처리할 수 있음
print("=" * 60)
...
# * to_numeric(error="coerce")  : 숫자로 변경이 불가능한 값에 대해 오류 대신 NaN으로 반환
for col in ['close', 'volume']:
    na_reult = pd.to_numeric(df2[col], errors="coerce").isna().sum()
    print(f"{col:<10} : {na_reult:>6}건")
...
#20260918 --> 8글자
print(f"20260918 형식 : {lens.get(8)}")
