...
# 마지막 거래일 -> 거래일: date / 최댓값(최근날짜)
last_day = df['date'].max()
print(f"마지막 거래일 : {df['date'].max()}")

# 마지막 거래일의 기록 -> 거래일이 마지막거래일과 일치하는 항목
...
# print(df['close'].between(10_000, 50_000))
result = df[df['close'].between(10_000, 50_000)]
print(f" 조회결과 : {len(result)}")

print(" === 종목코드가 'G0001', 'G0050', 'G1000'만 조회 ===")
result = df[df['code'].isin(['G0001', 'G0050', 'G1000'])]
print(result)
